...
if [[ "$1" == "--uninstall" ]]; then
    ...
    LINK_PATH="$HOME/.local/bin/poc-gpt-infosec-consultant"
    if [ -L "$LINK_PATH" ]; then
        rm "$LINK_PATH"
...
if [ -L "$LINK_PATH" ]; then
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ln -sf "$(pwd)/poc-gpt-infosec-consultant-runner.sh" "$LINK_PATH"
    ...
    ln -s "$(pwd)/poc-gpt-infosec-consultant-runner.sh" "$LINK_PATH"
...
chmod +x poc-gpt-infosec-consultant-runner.sh

echo "Installation complete. You can now use poc-gpt-infosec-consultant from anywhere!"
